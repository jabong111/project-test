...
<%
	...
	ListArticleService listService = ListArticleService.getInstance();	//서비스 객체를 만들고
	ArticleListModel articleListModel = listService.getArticleList(pageNumber);
	request.setAttribute("listModel", articleListModel); //ArticleListModel 객체를 listModel이름으로 영역에 저장한다.
	
	if(articleListModel.getTotalPageCount()>0){		//페이지가 0보다 크면
