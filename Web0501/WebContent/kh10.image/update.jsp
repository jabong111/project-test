...
<%
	...
	FileUploadRequestWrapper requestWrap = new FileUploadRequestWrapper(request, -1, -1,"C:\\Java\\App\\Web0426\\WebContent\\temp");
...
<%
	...
	if (theme.getPassword() == null || oldTheme.getPassword().compareTo(theme.getPassword()) == 0) {
		...
		if (imageFileItem.getSize() > 0) {
			...
			File imageFile = new File("C:\\Java\\App\\Web0426\\WebContent\\image", image);
			
			if (imageFile.exists()) {
				for (int i = 0; true; i++) {
					imageFile = new File("C:\\Java\\App\\Web0426\\WebContent\\image", "(" + i + ")" + image);
					if (!imageFile.exists()) {
						image = "(" + i + ")" + image;
						break;
					}
