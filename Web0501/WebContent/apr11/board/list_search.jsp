...
<%
	...
	int searchn = 0; //찾아줄 키워드
	String search = request.getParameter("search"); //찾을 단어
	System.out.println("search: "+search);
	if(pageNum == null){
		pageNum = "1";
	}
	
	if(search != null){	//검색할 단어가 있으면
	...
	if(search != null){	//찾는게 없으면
		count = dbPro.getArticleCount(searchn, search);	//총글의수
	}else{ 
...
<%}else{ //글이 있다면 %>
	...
	<%
		int wid=0;
		if(article.getRe_level()>0){	//0보다 크면 답글이면
	...
	<%} %>
		...
		<% if(comment_count > 0){ %>	<!-- 댓글이 있으면 댓글개수를 화면에 나오게하고  -->
...
<%
	if(count>0){	//글이 있으면
		...
		if(startPage>5){%>
			<%if(search != null){ %>
				<a href="list_search.jsp?pageNum=<%=startPage - 5%>&searchn=<%=searchn%>&search=<%=search%>">[이전]</a>
			<%}else{ %>
				<a href="list_search.jsp?pageNum=<%=startPage - 5%>">[이전]</a>
			<%} %>
<%		} 
		...
		if(endPage < pageCouunt){	%>
			<%if(search != null){ %>
				<a href="list_search.jsp?pageNum=<%=startPage+5%>&searchn=<%=searchn%>&search=<%=search%>">[다음]</a>
			<%}else{ %>
				<a href="list_search.jsp?pageNum=<%=startPage+5%>">[다음]</a>
			<%} %>
	<%} %>	
<%}%>
<%if(session.getAttribute("memId") != null){ %>
